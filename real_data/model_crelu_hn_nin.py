...
class BNN_SFM_CRELU_HN_NIn(PyroModule):
    def __init__(
        self,
        n_inputs: int = N_INPUTS,
        n_hidden1: int = N_HIDDEN1,
        n_hidden2: int = N_HIDDEN2,
        prior_scale_weight: float = SCALE_HALFNORMAL,
        ...
        self.fc1.weight = PyroSample(
            dist.HalfNormal(prior_scale_weight).expand([n_hidden1, n_inputs]).to_event(2)
        )
        self.fc1.bias = PyroSample(
            dist.Normal(0.0, prior_scale_bias).expand([n_hidden1]).to_event(1)
        )
        self.fc2.weight = PyroSample(
            dist.HalfNormal(prior_scale_weight).expand([n_hidden2, n_hidden1]).to_event(2)
        )
        self.fc2.bias = PyroSample(
            dist.Normal(0.0, prior_scale_bias).expand([n_hidden2]).to_event(1)
        )
        self.fc3.weight = PyroSample(
            dist.HalfNormal(prior_scale_weight).expand([1, n_hidden2]).to_event(2)
        )
    ...
    def forward(self, x, y=None):
        ...
        sigma2_u = pyro.sample("sigma2_u_bnn_crelu_hn",
                               dist.InverseGamma(self.alpha_u, self.beta_u))
        sigma2_v = pyro.sample("sigma2_v_bnn_crelu_hn",
                               dist.InverseGamma(self.alpha_v, self.beta_v))
        sigma_u = torch.sqrt(sigma2_u)
        sigma_v = torch.sqrt(sigma2_v)

        with pyro.plate("data_bnn_crelu_hn", n):
            u = pyro.sample("u_bnn_crelu_hn", dist.HalfNormal(sigma_u))
            pyro.sample("y_obs_bnn_crelu_hn",

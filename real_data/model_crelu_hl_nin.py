...
from config_real import (
    ...
    SCALE_HALFLAPLACE, SCALE_LAPLACE, N_INPUTS,
)


def _half_laplace(scale: Union[torch.Tensor, float]):
    return dist.FoldedDistribution(
        dist.Laplace(torch.as_tensor(0.0), torch.as_tensor(scale)),
        validate_args=False,
    )


def _laplace(scale: Union[torch.Tensor, float]):
    return dist.Laplace(torch.as_tensor(0.0), torch.as_tensor(scale))


class BNN_SFM_CRELU_HalfLaplace_NIn(PyroModule):
    def __init__(
        self,
        n_inputs: int = N_INPUTS,
        n_hidden1: int = N_HIDDEN1,
        n_hidden2: int = N_HIDDEN2,
        prior_scale_weight: float = SCALE_HALFLAPLACE,
        prior_scale_bias: float = SCALE_LAPLACE,
        a: float = 0.8,
        ...
        self.fc1.weight = PyroSample(
            _half_laplace(prior_scale_weight).expand([n_hidden1, n_inputs]).to_event(2)
        )
        self.fc1.bias = PyroSample(
            _laplace(prior_scale_bias).expand([n_hidden1]).to_event(1)
        )
        self.fc2.weight = PyroSample(
            _half_laplace(prior_scale_weight).expand([n_hidden2, n_hidden1]).to_event(2)
        )
        self.fc2.bias = PyroSample(
            _laplace(prior_scale_bias).expand([n_hidden2]).to_event(1)
        )
        self.fc3.weight = PyroSample(
            _half_laplace(prior_scale_weight).expand([1, n_hidden2]).to_event(2)
        )
        self.fc3.bias = PyroSample(
            _laplace(prior_scale_bias).expand([1]).to_event(1)
        )
    ...
    def forward(self, x, y=None):
        ...
        sigma2_u = pyro.sample("sigma2_u_bnn_crelu_hl",
                               dist.InverseGamma(self.alpha_u, self.beta_u))
        sigma2_v = pyro.sample("sigma2_v_bnn_crelu_hl",
                               dist.InverseGamma(self.alpha_v, self.beta_v))
        sigma_u = torch.sqrt(sigma2_u)
        sigma_v = torch.sqrt(sigma2_v)

        with pyro.plate("data_bnn_crelu_hl", n):
            u = pyro.sample("u_bnn_crelu_hl", dist.HalfNormal(sigma_u))
            pyro.sample("y_obs_bnn_crelu_hl",
                        dist.Normal(mu - u, sigma_v), obs=y)

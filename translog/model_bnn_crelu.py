...
class BNN_SFM_CRELU_2In(PyroModule):
    ...
    def __init__(
        ...
        n_hidden1=N_HIDDEN1,
        n_hidden2=N_HIDDEN2,
        prior_scale_weight=SCALE_HALFNORMAL,
        prior_scale_bias=SCALE_NORMAL,
        a=0.8,
        alpha_u=ALPHA_U,
        alpha_v=ALPHA_V,
        beta_u=BETA_U,
        ...
        self.fc1.weight = PyroSample(
            dist.HalfNormal(prior_scale_weight).expand([n_hidden1, 2]).to_event(2)
        )
        self.fc1.bias = PyroSample(
            dist.HalfNormal(prior_scale_bias).expand([n_hidden1]).to_event(1)
        )
        self.fc2.weight = PyroSample(
            dist.HalfNormal(prior_scale_weight)
            .expand([n_hidden2, n_hidden1])
            .to_event(2)
        )
        self.fc2.bias = PyroSample(
            dist.HalfNormal(prior_scale_bias).expand([n_hidden2]).to_event(1)
        )
